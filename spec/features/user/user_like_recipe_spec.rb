...
feature 'User like a recipe ' do
  scenario 'user see like button in your own recipes' do
    ...
    expect(page).to have_content(t('recipes.buttons.like_recipe'))
  end
  ...
  scenario 'user can just like one time' do
    ...
    visit recipe_path(recipe1)
    expect(page).to have_content t('recipes.buttons.like_recipe')
    click_link t('recipes.buttons.like_recipe')

    expect(page).to have_content t('recipes.buttons.unlike_recipe')
  ...
  scenario 'user can just unlike one time' do
    ...
    visit recipe_path(recipe1)
    expect(page).to have_content t('recipes.buttons.unlike_recipe')
    click_link t('recipes.buttons.unlike_recipe')
