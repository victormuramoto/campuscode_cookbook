...
feature 'user see recipes by food type' do
  ...
  scenario 'user see list of recipes available by food preference' do
    ...
    expect(page).to have_content recipe1.food_type1.name
    expect(page).to have_content recipe2.name
    expect(page).to have_content recipe2.food_type1.name
    expect(page).to have_content recipe3.name
    expect(page).to have_content recipe3.food_type1.name

    expect(page).to_not have_content recipe4.name
    expect(page).to_not have_content recipe4.food_type2.name
    expect(page).to_not have_content recipe5.name
    expect(page).to_not have_content recipe5.food_type2.name
    expect(page).to_not have_content recipe6.name
    expect(page).to_not have_content recipe6.food_type2.name
  end
end
